#!/usr/bin/env python3
"""Add CATEGORY_ICONS to LandingPage and use them in playground + mock dashboard."""

f = 'src/components/LandingPage.tsx'
code = open(f).read()

# Add CATEGORY_ICONS after CATEGORIES constant in LandingPage
icons_block = '''
...
# Insert after the CATEGORIES closing };
# Find the CATEGORIES block end
import re
match = re.search(r'(const CATEGORIES:.*?^};)', code, re.MULTILINE | re.DOTALL)
if match:
    insert_pos = match.end()
    code = code[:insert_pos] + '\n' + icons_block + code[insert_pos:]

# In playground category buttons, add icon before label
code = code.replace('{cat.label}', '{CATEGORY_ICONS[key]} {cat.label}')

open(f, 'w').write(code)
print('Done - LandingPage icons added')
